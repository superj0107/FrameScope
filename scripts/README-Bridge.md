...
推荐直接双击仓库根目录的 **`启动FrameScope桥接服务.bat`**。停止时双击 **`停止FrameScope桥接服务.bat`**。启动器会自动识别唯一的已授权 ADB 手表；连接多个设备时会提示输入序列号，运行结束后按任意键关闭窗口。
...
启动脚本使用 `app_process` 从 FrameScope APK 中加载 `ShellBridgeMain`。因为启动者是 `adb shell`，该进程拥有 Shell UID；FrameScope 主 App 仍然保持普通 App UID，双方通过共享存储中的请求/响应文件通信，不依赖网络或 Unix Socket 权限。FPS 监测会在每次切换前台应用时自动探测可用后端：优先使用有效的 `SurfaceFlinger --latency` 数据，若当前 ROM 返回无效数据，则使用前台应用的 `gfxinfo framestats` 完成帧统计。静止页面没有新帧时 FPS 显示 0，属于真实状态。
