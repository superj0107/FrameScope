...
if "!DEVICE_COUNT!"=="0" (
    echo No authorized ADB device found. Connect the watch and allow USB debugging.
    goto :finish
)
...
echo Starting FrameScope Bridge: !SERIAL!
call powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\start-framescope-bridge.ps1" -Serial "!SERIAL!"
if errorlevel 1 (
    echo Start failed. See the error above.
) else (
    echo.
    echo Start command sent. Open FrameScope and check the Bridge status.
)
